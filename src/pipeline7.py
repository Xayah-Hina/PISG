...
if __name__ == '__main__':
    ...
    for _1 in tqdm.trange(0, 1):
        ...
        for _2, (batch_points, batch_depths, batch_indices, batch_rays_d) in enumerate(tqdm.tqdm(sample_frustum(dirs=dirs, poses=poses, near=near[0].item(), far=far[0].item(), depth=depth_size, batch_size=batch_size, randomize=True, device=target_device, dtype=target_dtype))):
            ...
            batch_input_xyzt_flat = torch.cat([batch_points, batch_time.expand(batch_points[..., :1].shape)], dim=-1).reshape(-1, 4)
            bbox_mask = BBOX_MODEL_gpu.insideMask(batch_input_xyzt_flat, to_float=False)
            batch_input_xyzt_flat_final = batch_input_xyzt_flat[bbox_mask]

            batch_input_xyzt_flat_final.requires_grad = True
            ...
            h = encoder_d(batch_input_xyzt_flat_final)
            raw_d = model_d(h)

            jac = torch.vmap(torch.func.jacrev(g))(h)
            jac_x = _get_minibatch_jacobian(h, batch_input_xyzt_flat_final)
            jac = jac @ jac_x

            _d_x, _d_y, _d_z, _d_t = [torch.squeeze(_, -1) for _ in jac.split(1, dim=-1)]

            out_dim = 1
            raw_flat = torch.zeros([batch_input_xyzt_flat.shape[0], out_dim], device=target_device, dtype=target_dtype)
            raw_flat[bbox_mask] = raw_d
            raw = raw_flat.reshape(-1, depth_size, out_dim)

            dists_vals = batch_depths[..., 1:] - batch_depths[..., :-1]
            dists_cat = torch.cat([dists_vals, torch.tensor([1e10], device=target_device, dtype=target_dtype).expand(dists_vals[..., :1].shape)], dim=-1)
            batch_rays_d_flat = batch_rays_d.reshape(-1, 3)
            dists_final = dists_cat * torch.norm(batch_rays_d_flat[..., None, :], dim=-1)
            ...
            raw_vel, raw_f = model_v(encoder_v(batch_input_xyzt_flat_final))
            ...
            if torch.stack(nse_errors).sum() > 10000:
                print(f'skip large loss {torch.stack(nse_errors).sum():.3g}, timestep={batch_input_xyzt_flat_final[0, 3]}')
                continue
            nseloss_fine = 0.0
            split_nse_wei = [0.001]
        ...
        path = os.path.join("checkpoint", 'den_{:06d}.tar'.format(_1))
        torch.save({
        ...
        path = os.path.join("checkpoint", 'vel_{:06d}.tar'.format(_1))
        torch.save({
            'global_step': global_step,
