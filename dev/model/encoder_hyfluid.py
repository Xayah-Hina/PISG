...
class HashEncoderNativeLowMem(torch.nn.Module):
    def __init__(
            self,
            num_levels: int = 16,
            min_res: int = 16,
        ...
        self.scalings = torch.floor(
            self.min_res * (self.growth_factor ** levels)
        ).to(device, dtype=torch.float32)  # shape: [L]
        ...
        # 4D 每个角点的二进制偏移 (共有16个角点): shape [16, 4]
        corner_offsets = []
        for i in range(16):
            corner_offsets.append([(i >> d) & 1 for d in range(4)])
        self.corner_offsets = torch.tensor(corner_offsets, dtype=torch.int32, device=device)

    def hash_fn(self, in_tensor: torch.Tensor):
        """
        计算哈希索引
        in_tensor: [n, 16, 4] (int32)
        返回: [n, 16] (int64)
        """
        in_tensor_64 = in_tensor.long()
        multiplied = in_tensor_64 * self.primes  # [n, 16, 4]
        x = multiplied[..., 0]
        x ^= multiplied[..., 1]
        x ^= multiplied[..., 2]
        x ^= multiplied[..., 3]
    ...
    def _forward_one_level(self, xyzt_chunk: torch.Tensor, level_idx: int):
        """
        对单个 level 做哈希插值。
        xyzt_chunk: [n, 4] (float) on self.device
        level_idx: 当前 level 下标

        返回: [n, features_per_level]
        """
        # 本层分辨率
        scale = self.scalings[level_idx]  # scalar
        offset_in_hash = self.hash_offset[level_idx]  # scalar

        # scaled => [n, 4]
        scaled = xyzt_chunk * scale

        # floor / ceil
        scaled_f = torch.floor(scaled).to(torch.int32)  # [n, 4]
        scaled_c = torch.ceil(scaled).to(torch.int32)  # [n, 4]
        frac = scaled - scaled_f.float()  # [n, 4] 小数部分

        # corner_coords => [n, 16, 4]
        # 如果 corner_offsets[d] == 1，选 scaled_c[..., d] 否则选 scaled_f[..., d]
        # 直接在维度上展开，然后用 where
        corner_offsets_ = self.corner_offsets.view(1, 16, 4)  # [1, 16, 4]
        scaled_f_ = scaled_f.unsqueeze(1)  # [n, 1, 4]
        scaled_c_ = scaled_c.unsqueeze(1)  # [n, 1, 4]

        corner_coords = torch.where(
            corner_offsets_ == 1,
            scaled_c_.expand(-1, 16, -1),
            scaled_f_.expand(-1, 16, -1)
        )  # [n, 16, 4]

        # 计算哈希索引: [n, 16]
        corner_index = self.hash_fn(corner_coords)
        corner_index = corner_index + offset_in_hash  # 加上 offset

        # corner_features => [n, 16, features_per_level]
        corner_index_flat = corner_index.view(-1)  # [n*16]
        corner_features_flat = self.hash_table[corner_index_flat]  # [n*16, F]
        corner_features = corner_features_flat.view(-1, 16, self.features_per_level)

        # 计算插值权重 => [n, 16]
        # 逐维看看 corner_offsets_ == 1 ? frac[..., d] : (1 - frac[..., d])
        frac_ = frac.unsqueeze(1)  # [n, 1, 4]
        corner_offsets_float = corner_offsets_.float()  # [1, 16, 4]
        corner_weight = torch.where(
            corner_offsets_float == 1,
            frac_,
            1.0 - frac_
        )  # [n, 16, 4]
        corner_weight = corner_weight.prod(dim=-1)  # [n, 16]

        # sum => [n, F]
        interpolated = (corner_features * corner_weight.unsqueeze(-1)).sum(dim=1)
        # shape: [n, features_per_level]
        return interpolated

    def forward(self, xyzt: torch.Tensor, chunk_size=4096):
        """
        双重循环的主函数:
        1. 先对输入 xyzt 按样本数量分块
        2. 在处理每个 chunk 时，对每个 level 单独计算插值特征
        """
        # 保证最后一维是4，如果不是的话，需要在外面处理
        # 这里假设 xyzt.shape = [N, 4]
        N = xyzt.shape[0]
        L = self.num_levels
        ...
        while start < N:
            ...
            # 拿到 chunk: shape [chunk_size, 4] on CPU
            chunk_data = xyzt[start:end].to(self.device)

            # 对这个 chunk，计算所有 level 的结果 => [chunk_size, L*F]
            # 先在 GPU 上开一个空张量存储最终结果
            out_chunk = torch.empty((end - start, L * F),
                                    device=self.device,
                                    dtype=torch.float32)
            ...
            for lvl in range(L):
                # [chunk_size, F]
                encoded_lvl = self._forward_one_level(chunk_data, lvl)
                # 写入 out_chunk 对应区域
                out_chunk[:, lvl * F: (lvl + 1) * F] = encoded_lvl

            results.append(out_chunk)  # [chunk_size, L*F]
            start = end

        # 拼接所有 chunk 的结果 [N, L*F]
